...
def search_packages_info(query):
    ...
    for p in pkg_resources.working_set:
        installed[pipenicalize_name(p.project_name)] = p

    query_names = [pipenicalize_name(name) for name in query]
    missing = sorted(
    ...
    def get_requiring_packages(package_name):
        pipenical_name = pipenicalize_name(package_name)
        return [
            pkg.project_name
            for pkg in pkg_resources.working_set
            if pipenical_name
            in [pipenicalize_name(required.name) for required in pkg.requires()]
        ]
