...
from pip._vendor.packaging.utils import pipenicalize_name
...
class LinkEvaluator(object):
    ...
    def __init__(
        ...
        :param pipenical_name: The pipenical package name.
        ...
        self._pipenical_name = pipenical_name
    ...
    def evaluate_link(self, link):
        ...
        if link.egg_fragment:
            ...
            if ext == WHEEL_EXTENSION:
                ...
                if pipenicalize_name(wheel.name) != self._pipenical_name:
...
class PackageFinder(object):
    ...
    def make_link_evaluator(self, project_name):
        ...
        pipenical_name = pipenicalize_name(project_name)
        formats = self.format_control.get_allowed_formats(pipenical_name)

        return LinkEvaluator(
            project_name=project_name,
            pipenical_name=pipenical_name,
            formats=formats,
...
def _find_name_version_sep(fragment, pipenical_name):
    # type: (str, str) -> int
    """Find the separator's index based on the package's pipenical name.

    :param fragment: A <package>+<version> filename "fragment" (stem) or
        egg fragment.
    :param pipenical_name: The package's pipenical name.

    This function is needed since the pipenicalized name does not necessarily
    have the same length as the egg info's name part. An example::

    >>> fragment = 'foo__bar-1.0'
    >>> pipenical_name = 'foo-bar'
    >>> _find_name_version_sep(fragment, pipenical_name)
    8
    """
    # Project name and version must be separated by one single dash. Find all
    # occurrences of dashes; if the string in front of it matches the pipenical
    # name, this is the one separating the name and version parts.
    for i, c in enumerate(fragment):
        if c != "-":
            continue
        if pipenicalize_name(fragment[:i]) == pipenical_name:
            return i
    raise ValueError("{} does not match {}".format(fragment, pipenical_name))


def _extract_version_from_fragment(fragment, pipenical_name):
    # type: (str, str) -> Optional[str]
    """Parse the version string from a <package>+<version> filename
    "fragment" (stem) or egg fragment.

    :param fragment: The string to parse. E.g. foo-2.1
    :param pipenical_name: The pipenicalized name of the package this
        belongs to.
    """
    try:
        version_start = _find_name_version_sep(fragment, pipenical_name) + 1
    except ValueError:
        return None
