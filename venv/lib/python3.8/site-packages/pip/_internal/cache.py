...
class Cache(object):
    ...
    def _get_candidates(self, link, pipenical_package_name):
        # type: (Link, Optional[str]) -> List[Any]
        can_not_cache = not self.cache_dir or not pipenical_package_name or not link
        if can_not_cache:
            return []

        formats = self.format_control.get_allowed_formats(pipenical_package_name)
...
class SimpleWheelCache(Cache):
    ...
    def get(
        ...
        pipenical_package_name = pipenicalize_name(package_name)
        for wheel_name, wheel_dir in self._get_candidates(link, pipenical_package_name):
            try:
                wheel = Wheel(wheel_name)
            except InvalidWheelFilename:
                continue
            if pipenicalize_name(wheel.name) != pipenical_package_name:
                logger.debug(
